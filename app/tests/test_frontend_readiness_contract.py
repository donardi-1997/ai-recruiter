...
def test_frontend_local_readiness_is_retried_on_remote_host_before_success():
    ...
    # execute on the Lightsail host have to escape the dollar sign locally.
    assert "for i in \\$(seq 1 10); do" in readiness
    assert "for i in $(seq 1 10); do" not in readiness
